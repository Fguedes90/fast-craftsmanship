...
async def process_link(
    ...
    try:
        ...
        return make_error(
            ProcessingError,
            f"Failed to process link: {str(e)}"
        )
...
async def process_page_links(
    ...
    try:
        ...
        return make_error(
            ProcessingError,
            f"Failed to process page links: {str(e)}"
        )
...
async def is_url_processed(url: Url, tracker: UrlTracker) -> Result[bool, Exception]:
    ...
    try:
        ...
        return make_error(
            ProcessingError,
            f"Failed to check URL status: {str(e)}"
        )
...
async def mark_url_visited(url: Url, tracker: UrlTracker) -> ScraperResult:
    ...
    try:
        ...
        return make_error(
            ProcessingError,
            f"Failed to mark URL as visited: {str(e)}"
        )
...
async def mark_url_failed(url: Url, error: str, tracker: UrlTracker) -> ScraperResult:
    ...
    try:
        ...
        return make_error(
            ProcessingError,
            f"Failed to mark URL as failed: {str(e)}"
        )