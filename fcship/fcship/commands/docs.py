...
def docs(
    ...
    content_selector: Optional[str] = typer.Option(None, "--selector", "-s", help="CSS selector for content extraction"),
    timeout: Optional[float] = typer.Option(30.0, "--timeout", "-t", help="Timeout for page operations in seconds")
) -> None:
    """Scrape documentation from websites using Railway Oriented Programming."""
    if operation != "scrape":
        error_message(
            f"Unknown operation: {operation}",
            "Available operations: scrape"
        )
        raise typer.Exit(1)
