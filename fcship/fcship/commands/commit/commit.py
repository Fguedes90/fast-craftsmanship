...
from .utils import GitCommands, GitStatus, GitFileStatus
from .generate_commit_message import generate_commit_message, combine_commit_messages
...
def commit_interactive() -> None:
    ...
    if option.upper() == "M":
        ...
        for status_type in ['renamed', 'added', 'modified', 'deleted', 'untracked']:
            ...
            for file in files:
                answer = Prompt.ask(f"Include '{file.display_path}'?", choices=["y", "n"], default="y")
                if answer.lower() == "y":
