...
def validate_domain_operation(operation: str):
    ...
    if operation not in valid_operations:
        yield Error(DomainError.ValidationError(
...
def prepare_domain_context(name: str):
    ...
    try:
        ...
        # Create context
        yield Ok(DomainContext(
            name=name,
            files=files_map
        ))
    except Exception as e:
        yield Error(DomainError.TemplateError(f"Failed to generate domain templates: {str(e)}"))
...
def create_domain_file(domain_name: str, file_path: str, content: str):
    ...
    try:
        ...
        if dir_result.is_error():
            yield Error(DomainError.FileError(
        ...
        yield Error(DomainError.FileError(
            f"domain/{domain_name}/{file_path}",
...
def create_domain_files(ctx: DomainContext):
    ...
    try:
        ...
        yield Error(DomainError.FileError(
            f"domain/{ctx.name}",
            f"Failed to create domain files: {str(e)}"
        ))
