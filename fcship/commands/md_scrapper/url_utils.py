...
def validate_url(url: Url, base_url: str, allowed_paths: List[str]) -> Result[bool, Exception]:
    ...
    try:
        ...
        return make_error(
            ValidationError,
            f"Failed to validate URL {url}: {str(e)}"
        )
...
def normalize_url(url: str) -> Result[Url, Exception]:
    ...
    try:
        ...
        return make_error(
            ValidationError,
            f"Failed to normalize URL {url}: {str(e)}"
        )
...
def join_urls(base: str, path: str) -> Result[Url, Exception]:
    ...
    try:
        ...
        return make_error(
            ValidationError,
            f"Failed to join URLs {base} and {path}: {str(e)}"
        )
...
def extract_base_url(url: str) -> Result[Url, Exception]:
    ...
    try:
        ...
        return make_error(
            ValidationError,
            f"Failed to extract base URL from {url}: {str(e)}"
        )
...
def get_safe_filename(url: str, is_markdown: bool = False) -> Result[str, Exception]:
    ...
    try:
        ...
        return make_error(
            ValidationError,
            f"Failed to generate safe filename for {url}: {str(e)}"
        )