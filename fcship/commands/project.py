...
from expression import Result, Ok, Error, effect, tagged_union, pipe
from expression.collections import Map, seq, Block
...
from fcship.utils.file_utils import write_file, ensure_directory
...
class ProjectError:
    ...
    file_error: Optional[Tuple[str, str]] = None
...
def validate_project_operation(operation: str):
    ...
    if operation not in valid_operations:
        yield Error(ProjectError.ValidationError(
...
def validate_project_name(name: str):
    ...
    if not all(c.isalnum() or c in "-_" for c in name):
        yield Error(ProjectError.ValidationError("Project name must contain only alphanumeric characters, hyphens, or underscores"))
        return
...
def prepare_project_context(name: str):
    ...
    try:
        ...
        yield Error(ProjectError.TemplateError(f"Failed to generate project templates: {str(e)}"))
...
def create_project_directory(ctx: ProjectContext, folder: str):
    ...
    try:
        ...
        yield Error(ProjectError.FileError(
            str(ctx.root_path / folder),
...
def create_project_directories(ctx: ProjectContext):
    ...
    try:
        ...
        yield Error(ProjectError.FileError(
            str(ctx.root_path),
            f"Failed to create project directories: {str(e)}"
        ))
...
"""
        ...
        yield Error(ProjectError.ValidationError(f"Failed to display next steps: {str(e)}"))
