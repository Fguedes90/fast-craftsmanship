...
def init_repo(
    ...
    auto_init: Annotated[bool, typer.Option(help="Initialize with README, LICENSE, and .gitignore")] = True,
    license_template: Annotated[Optional[str], typer.Option(help="License template to use (e.g., mit, apache-2.0)")] = "mit",
    gitignore_template: Annotated[Optional[str], typer.Option(help="Gitignore template to use (e.g., Python, Node)")] = "Python",
):
    """Create a new GitHub repository with best practices."""
    
    result = pipe(
        _create_repository(repo_name, description, private, auto_init, license_template, gitignore_template),
        lambda _: display_success(f"Repository {repo_name} created successfully.")
    )
...
def protect_branch(
    ...
    required_approvals: Annotated[int, typer.Option(help="Number of required approvals for PRs")] = 1,
    require_status_checks: Annotated[bool, typer.Option(help="Require status checks to pass")] = True,
    ...
    result = pipe(
        ...
        lambda _: display_success(f"Branch protection set up for {branch_name} on {repo_name}")
    )
...
def setup_secrets(
    ...
    result = pipe(
        ...
        lambda _: display_success(f"Secrets configured for {repo_name}")
    )
...
def setup_environments(
    repo_name: Annotated[str, typer.Argument(help="Name of the repository")],
    environments: Annotated[List[str], typer.Option(help="Environments to set up")] = ["staging", "production"],
    require_approvals: Annotated[bool, typer.Option(help="Require approvals for deployments")] = True,
):
    """Set up GitHub environments for deployments."""
    
    result = pipe(
        _setup_repository_environments(repo_name, environments, require_approvals),
        lambda _: display_success(f"Environments {', '.join(environments)} set up for {repo_name}")
    )
...
def setup_workflows(
    ...
    result = pipe(
        ...
        lambda _: display_success(f"Workflows set up for {repo_name}")
    )
...
def setup_all(
    ...
    deployment_environments: Annotated[List[str], typer.Option(help="Environments to set up")] = ["staging", "production"],
):
    """Complete GitHub repository setup with best practices."""
    
    # Step 1: Create repository
    create_repo_result = yield from _create_repository(
    ...
    protection_result = yield from _setup_branch_protection(
        repo_name, 
        BranchProtectionRule(branch_name="main", required_approvals=1)
    )
    ...
    secrets_result = yield from _setup_repository_secrets(
        repo_name, 
    ...
    env_result = yield from _setup_repository_environments(
        repo_name,
    ...
    workflow_result = yield from _setup_github_workflows(
        repo_name,
        ci=True,
        release=True,
        version_bump=True,
