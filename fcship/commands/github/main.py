...
@effect.result[None, str]()
def create_release(token: str, repo_name: str, release: Release, display_ctx: DisplayContext = None):
    ...
    try:
        ...
        try:
            ...
            yield from error_message(display_ctx, f"Error creating release for repository '{repo_name}'", details=str(e))
            return Error(str(e))
...
@effect.result[None, str]()
def list_pull_requests(token: str, repo_name: str, state: str = "open", display_ctx: DisplayContext = None):
    ...
    try:
        ...
        try:
            ...
            # Create and display table
            table_result = yield from create_multi_column_table(
                f"{state_display} Pull Requests in {repo_name}",
                headers,
                rows
            )
            if table_result.is_ok():
            ...
            yield from error_message(display_ctx, f"Error listing pull requests for repository '{repo_name}'", details=str(e))
            return Error(str(e))
...
@effect.result[None, str]()
def create_issue(token: str, repo_name: str, title: str, body: str, labels: List[str] = None, display_ctx: DisplayContext = None):
    ...
    try:
        ...
        try:
            ...
            yield from error_message(display_ctx, f"Error creating issue for repository '{repo_name}'", details=str(e))
            return Error(str(e))
...
def get_recent_workflow_runs(token: str, repo_name: str, limit: int = 10, branch: str = None, status: str = None):
    ...
    try:
        ...
        yield Error(f"Failed to get workflow runs: {str(e)}")

@effect.result[None, str]()
def list_workflow_runs(token: str, repo_name: str, limit: int = 10, branch: str = None, status: str = None, display_ctx: DisplayContext = None):
    ...
    try:
        ...
        yield from display_rule(display_ctx, f"GitHub Actions Workflow Runs for {repo_name}{branch_display}")
...
def get_workflow_run_details(token: str, repo_name: str, run_id: int):
    ...
    try:
        ...
        yield Error(f"Failed to get workflow run details: {str(e)}")

@effect.result[None, str]()
def display_workflow_run_details(token: str, repo_name: str, run_id: int, display_ctx: DisplayContext = None):
    ...
    try:
        ...
        if run_result.is_error():
            yield from error_message(display_ctx, "Failed to get workflow run details", run_result.error)
        ...
        yield from success_message(display_ctx, f"Status: {run['status']} / Conclusion: {run['conclusion'] or 'pending'}")
        yield from success_message(display_ctx, f"Branch: {run['head_branch']} ({run['head_sha'][:7]})")
        yield from success_message(display_ctx, f"Triggered by: {run['event'].replace('_', ' ').title()}")
        yield from success_message(display_ctx, f"Started: {run['created_at'].strftime('%Y-%m-%d %H:%M:%S UTC')}")
        ...
        jobs_table_result = yield from create_multi_column_table(
            "Jobs",
            job_headers,
            job_rows
        )
        ...
        for job_index, job in enumerate(run["jobs"]):
            ...
            if steps_table_result.is_ok():
                ...
                yield from error_message(display_ctx, f"Failed to create steps table for job {job['name']}")
                
    except Exception as e:
        yield from error_message(display_ctx, "Failed to display workflow run details", details=str(e))
        return Error(str(e))
...
def get_workflow_logs(token: str, repo_name: str, run_id: int):
    ...
    try:
        ...
        with Progress(
            SpinnerColumn(),
            TextColumn("[bold blue]Downloading workflow logs...")
        ) as progress:
        ...
        yield Error(f"Failed to download workflow logs: {str(e)}")
...
@effect.result[None, str]()
def display_workflow_logs(token: str, repo_name: str, run_id: int, show_failed_only: bool = True, display_ctx: DisplayContext = None):
    ...
    try:
        ...
        if show_failed_only:
            ...
            for step_name, step_log in failed_logs.items():
                ...
                panel = Panel(
                    syntax,
                    title=f"[bold red]Failed Step: {step_name}[/bold red]",
                    expand=False
                )
...
def rerun_workflow(token: str, repo_name: str, run_id: int):
    ...
    try:
        ...
        yield Error(f"Failed to rerun workflow: {str(e)}")
...
def watch_workflow_run(token: str, repo_name: str, run_id: int, display_ctx: DisplayContext = None):
    ...
    try:
        ...
        with Progress(
            SpinnerColumn(),
            TextColumn("[bold blue]Watching workflow run..."),
            transient=True
        ) as progress:
            task = progress.add_task("watching", total=None)
            
            # Track job states
            job_states = {job["id"]: {"status": job["status"], "conclusion": job["conclusion"]} for job in run["jobs"]}
            ...
            while active and (time.time() - start_time) < max_duration:
                ...
                if time.time() - last_refresh >= update_interval:
                    ...
                    if current_run.status != initial_run.status or current_run.conclusion != initial_run.conclusion:
                        display_ctx.console.print(f"[yellow]Workflow status changed: {initial_run.status} → {current_run.status} / {initial_run.conclusion or 'pending'} → {current_run.conclusion or 'pending'}[/yellow]")
                    ...
                    for job in run["jobs"]:
                        ...
                        if job_id in job_states:
                            ...
                            if job["status"] != old_status or job["conclusion"] != old_conclusion:
                                display_ctx.console.print(f"[cyan]Job '{job['name']}' changed: {old_status} → {job['status']} / {old_conclusion or 'pending'} → {job['conclusion'] or 'pending'}[/cyan]")
                                job_states[job_id] = {"status": job["status"], "conclusion": job["conclusion"]}
                        else:
                            display_ctx.console.print(f"[green]New job started: '{job['name']}' - {job['status']}[/green]")
                            job_states[job_id] = {"status": job["status"], "conclusion": job["conclusion"]}
        ...
        if final_run_result.is_ok():
            ...
            yield from success_message(display_ctx, f"Status: [{status_color}]{final_run['status']}[/{status_color}]")
            yield from success_message(display_ctx, f"Conclusion: [{status_color}]{final_run['conclusion'] or 'pending'}[/{status_color}]")
            
            if final_run["conclusion"] == "failure":
                yield from warning_message(display_ctx, "Workflow failed! Use 'logs' command to see error details")
        else:
            # Use the last known state
            status_color = "green" if initial_run.conclusion == "success" else "red"
            yield from success_message(display_ctx, f"Status: [{status_color}]{initial_run.status}[/{status_color}]")
            yield from success_message(display_ctx, f"Conclusion: [{status_color}]{initial_run.conclusion or 'pending'}[/{status_color}]")
