...
def validate_test_type(test_type: str):
    ...
    if test_type not in valid_types:
        yield Error(TestError.ValidationError(
...
def prepare_test_context(test_type: str, name: str):
    ...
    try:
        ...
        yield Error(TestError.ValidationError(f"Failed to prepare test context: {str(e)}"))
...
def ensure_test_directory(ctx: TestContext):
    ...
    try:
        ...
        if result.is_error():
            yield Error(TestError.FileError(
                str(directory),
                f"Failed to create directory: {result.error}"
            ))
            return
...
def write_test_file(ctx: TestContext, display_ctx: DisplayContext = None):
    ...
    try:
        ...
        if result.is_error():
            yield Error(TestError.FileError(
                str(ctx.file_path),
                f"Failed to write file: {result.error}"
            ))
            return
