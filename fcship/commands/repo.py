...
def validate_repo_operation(operation: str):
    ...
    if operation not in valid_operations:
        yield Error(RepoError.ValidationError(
...
def prepare_repo_context(name: str):
    ...
    try:
        ...
        # Create context
        yield Ok(RepoContext(
            name=name,
            files=files_map
        ))
    except Exception as e:
        yield Error(RepoError.TemplateError(f"Failed to generate repository templates: {str(e)}"))
...
def create_repo_file(file_path: str, content: str):
    ...
    try:
        ...
        if dir_result.is_error():
            yield Error(RepoError.FileError(
                str(path.parent),
                f"Failed to create directory: {dir_result.error}"
            ))
            return
...
def create_repo_files(ctx: RepoContext):
    ...
    try:
        ...
        yield Error(RepoError.FileError(
            "repository",
            f"Failed to create repository files: {str(e)}"
        ))
