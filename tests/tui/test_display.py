...
def test_display_rule_with_error(mocker, display_ctx):
    ...
    mock_print_rule = mocker.patch('fcship.tui.display.print_rule', return_value=Error(DisplayError.Rendering("Rule error", Exception("Test"))))
...
def test_process_messages_with_display_error(valid_batch_messages, mocker, display_ctx):
    mock_display = mocker.patch('fcship.tui.display.display_message', return_value=Error(DisplayError.Rendering("Display error", Exception("Test"))))
    result = process_messages(display_ctx, valid_batch_messages)
    assert result.is_error()
...
def test_handle_display_print_error(display_ctx):
    ...
    with patch('fcship.tui.display.print_styled', return_value=Error(DisplayError.Rendering("Print error", Exception("Test")))):
